...
onEvent('item.registry.tool_tiers', event => {
  ...
  onEvent('item.registry', event => {
    ...
    event.create('custom_paper', item => {
        item.displayName('§b定制物品兑换券').tooltip(Text.of('§7右键使用(仅定制了物品的赞助者、挑战者有效)'))
      })
    
      event.create('singer_sword', item => {
        item.displayName('§6虚空の剑').tooltip(Text.of('歌者の剑'))
      ...
      event.create('bengdai', item => {
        item.displayName('恋人の绷带').tooltip(Text.of('          §7--@_Inory_'))
        item.rarity(Rarity.EPIC)
        item.unstackable()
      })
