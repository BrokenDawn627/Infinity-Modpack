...
onEvent('item.registry.tool_tiers', event => {
  ...
  onEvent('item.registry', event => {
    ...
    event.create('infinity_sword', item => {
        item.displayName('§1§l§ka§r§9无限支配之剑§1§l§ka§r').tooltip(Text.of('§7游戏结束'))
        item.tooltip('')
    ...
    event.create('infinity_sword_a', item => {
		item.displayName('§9无限支配之剑剑柄').tooltip(Text.of('§7制作材料')).unstackable()
    item.rarity(Rarity.EPIC)
	  })

      event.create('infinity_sword_b', item => {
		item.displayName('§9无限支配之剑剑身').tooltip(Text.of('§7制作材料')).unstackable()
    item.rarity(Rarity.EPIC)
	  })
