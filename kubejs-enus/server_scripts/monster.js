            ...
            for(let i=0;i<finalTags.length;i++)
            {
                ...
                if(entity.type.indexOf("creeper")<0)
                {
                    ...
                    if(finalTags[i]==1)
                    {
                        ...
                        entity.setCustomName("§bSpeed§r " + entity.name)
                    }
                    //2磐石-护甲+100%+10 但移动速度变慢 且无法被击退-33%
                    if(finalTags[i]==2)
                    {
                        entity.tags.add('panshi')
                        entity.setCustomName("§8Rock§r " + entity.name)
                    }
                    //6吸血-以造成伤害的100%回复生命值
                    if(finalTags[i]==6)
                    {
                        entity.tags.add('xixue')
                        entity.setCustomName("§4LifeSteal§r " + entity.name)
                    }
                    //7隐身
                    if(finalTags[i]==7)
                    {
                        entity.tags.add('yinshen')
                        entity.setCustomName("§7Invisible§r " + entity.name)
                    }
                }
                ...
                if(finalTags[i]==3)
                {
                    ...
                    entity.setCustomName("§cReturn§r " + entity.name)
                }
                //4致盲-玩家被攻击时会失明
                if(finalTags[i]==4)
                {
                    entity.tags.add('zhimang')
                    entity.setCustomName("§0Blinding§r " + entity.name)
                }
                //5寒霜-玩家被攻击时会虚弱并减速
                if(finalTags[i]==5)
                {
                    entity.tags.add('hanshuang')
                    entity.setCustomName("§9Frosting§r " + entity.name)
                }
                /*//8重生-怪物将持有不死图腾
                if(finalTags[i]==8)
                {
                    entity.tags.add('chongsheng')
                    entity.setCustomName("§6Respawn§r " + entity.name)
                }*/
            }
